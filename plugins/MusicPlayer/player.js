...
exports.play = {
		...
		if (arr.length == 0) return msg.channel.sendMessage( wrap('No estas en un canal de voz.'));

		// Make sure the suffix exists.
		if (!suffix) return msg.channel.sendMessage( wrap('No hay video especifico!'));
		...
		if (queue.length >= MAX_QUEUE_SIZE) {
			return msg.channel.sendMessage( wrap('Tamano de lista exedida!'));
		}

		// Get the video information.
		msg.channel.sendMessage( wrap('Buscando...')).then(response => {
			...
			YoutubeDL.getInfo(suffix, ['-q', '--no-warnings', '--force-ipv4'], (err, info) => {
				...
				if (err || info.format_id === undefined || info.format_id.startsWith('0')) {
					return response.edit( wrap('Video invalido!'));
				}

				// Queue the video.
				response.edit( wrap('En cola: ' + info.title)).then((resp) => {
					queue.push(info);
...
exports.skip = {
	...
	process:function(client, msg, suffix) {
		...
		if (voiceConnection === null) return msg.channel.sendMessage( wrap('No hay musica siguiente para reproducir.'));
		...
		msg.channel.sendMessage( wrap('Saltado ' + toSkip + '!'));
	}
}
...
exports.queue = {
	...
	process: function(client, msg, suffix) {
		...
		if (voiceConnection !== null && voiceConnection != undefined) {
			queueStatus = voiceConnection.paused ? 'Pausado' : 'Reproduciendo';
		}

		// Send the queue and status.
		msg.channel.sendMessage( wrap('Lista (' + queueStatus + '):\n' + text));
	}
}
...
exports.pause = {
	...
	process: function(client, msg, suffix) {
		// Get the voice connection.
		const voiceConnection = client.voiceConnections.get(msg.guild.id);
		if (voiceConnection == null) return msg.channel.sendMessage( wrap('No se reproduce musica.'));

		// Pause.
		msg.channel.sendMessage( wrap('Reproduccion detenida.'));
...
function executeQueue(client, msg, queue) {
		...
		new Promise((resolve, reject) => {
			...
			// Play the video.
			msg.channel.sendMessage( wrap('Reproduciendo ahora: ' + video.title)).then((cur) => {
