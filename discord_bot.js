...
var dangerousCommands = ["eval","pullanddeploy","setUsername","online"];
var Permissions = {};
try{
...
var commands = {
	...
	"gif": {
		...
		process: function(bot, msg, suffix) {
			...
			if (typeof id !== "undefined") {
			    ...
			    msg.channel.sendMessage( "Tag invalido, intenta con algo diferente. [Tags: " + (tags ? tags : "Random GIF") + "]");
			}
		    });
		}
	},
    "ping": {
		...
		"Hola": {
				description: "te saludo cuando lo saludas",
				process: function(bot, msg, suffix) {
						msg.channel.sendMessage( msg.author+" Holi!");
						if(suffix){
								msg.channel.sendMessage( "Como estas?!");
						}
				}
		},
    "myid": {
    ...
    "meme": {
        usage: 'meme "top text" "bottom text"',
            ...
            var str = "Memes disponibles actualmente:\n"
            for (var m in meme){
                str += "\t\t" + m + "\n"
            }
            return str;
    ...
    "wiki": {
        ...
        process: function(bot,msg,suffix) {
            var query = suffix;
            if(!query) {
                msg.channel.sendMessage("uso: " + Config.commandPrefix + "wiki termino de buscqueda");
                return;
            }
              ...
              fields: ['s', 'n', 'd1', 'l1', 'y', 'r'],
                ...
                if(error){
                    ...
                    msg.channel.sendMessage(snapshot.name
                        + "\nprecio: $" + snapshot.lastTradePriceOnly);
                }
            });
        }
    },
	"wolfram": {
        ...
        process: function(bot,msg,suffix){
				if(!suffix){
					msg.channel.sendMessage("Uso: " + Config.commandPrefix + "wolfram <termino de busqueda> (Ex. " + Config.commandPrefix + "wolfram integrate 4x)");
				}
        ...
        if(cmdTxt === "help"){
            //help is special since it iterates over the other commands
						if(suffix){
							...
							msg.author.sendMessage("**Comandos Disponibles:**").then(function(){
								var batch = "";
			...
			if(Permissions.checkPermission(msg.author,cmdTxt)){
				...
				msg.channel.sendMessage("No tienes permitido usar " + cmdTxt + "!");
			}
		} else {
			msg.channel.sendMessage(cmdTxt + " no es un comando conocido!").then((message => message.delete(5000)))
		}
	} else {
